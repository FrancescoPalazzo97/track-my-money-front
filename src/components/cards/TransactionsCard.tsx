...
const TransactionsCard = ({ transaction }: Props) => {
    ...
    return (
        <li
            onClick={handleClick}
            className='bg-slate-900/50 backdrop-blur-sm border border-slate-800/50 rounded-xl p-3 sm:p-4 hover:border-slate-700/50 transition-all duration-200 hover:shadow-lg hover:shadow-emerald-500/5 cursor-pointer group'
        >
            <div className='flex items-start justify-between gap-2 sm:gap-4'>
                <div className='flex-1 min-w-0 space-y-1.5 sm:space-y-2'>
                    <h3 className='text-slate-100 font-medium text-base sm:text-lg truncate group-hover:text-emerald-400 transition-colors'>
                        {transaction.title}
                    </h3>
                    ...
                    <div className='flex items-center gap-2 sm:gap-4 text-slate-500 text-[10px] sm:text-xs'>
